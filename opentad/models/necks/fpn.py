...
class FPNIdentity(nn.Module):
    ...
    def forward(self, inputs, fpn_masks):
        ...
        # 打印 feature maps
        def print_recursive(x, prefix=""):
            if isinstance(x, torch.Tensor):
                print(f"{prefix}Tensor shape: {x.shape}")
            elif isinstance(x, (list, tuple)):
                print(f"{prefix}{type(x)} with length {len(x)}")
                for i, item in enumerate(x):
                    print_recursive(item, prefix + f"[{i}]->")
            else:
                print(f"{prefix}Type: {type(x)}")

        print("FPN forward inputs type:", type(inputs))
        print_recursive(inputs)

        print("FPN forward masks type:", type(fpn_masks))
        print_recursive(fpn_masks)
